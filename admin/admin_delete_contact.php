...
if(isset($_GET['id']))
{
	...
	$sql	= "UPDATE contact_us SET status = 0, updated_date = '$date' WHERE id = '$id'";
	mysqli_query($con,$sql);
	if(mysqli_affected_rows($con) > 0)
	{
		setcookie('alert','alert-success',time()+4,'/');
		setcookie('sms','Contact us deleted successfully.',time()+4,'/');
		setcookie('undo_id',$id,time()+4,'/');
		header('location:http://localhost/jquery_ajax_crud/admin/contacts');
	}
	else
	{
		setcookie('alert','alert-danger',time()+4,'/');
		setcookie('sms','Contact us not deleted successfully.',time()+4,'/');
		header('location:http://localhost/jquery_ajax_crud/admin/contacts');
	}
}
// undo delete
